...
import { CheckCircle, Copy, Mail, Search, ShieldCheck, UserCog, UserPlus, Users } from 'lucide-react';
...
function buildCredentialText(account: CreateAdminAccountResult) {
  return [
    ...
    `Tijdelijk wachtwoord: ${account.temporaryPassword}`,
...
export function AdminAccountsPage({ adminEmail, isDarkMode }: AdminAccountsPageProps) {
  ...
  const handleFormChange = (field: 'displayName' | 'email', value: string) => {
  ...
  const handleCreateAdmin = async (event: React.FormEvent) => {
    ...
    try {
      const createdAdmin = await createAdminAccount({ displayName, email });
        ...
        <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
            ...
            <form onSubmit={handleCreateAdmin} className="space-y-5">
              <div>
                ...
                <p className={cn('mt-2 text-sm', isDarkMode ? 'text-[#a8b3c1]' : 'text-gray-500')}>
                  We maken een echt adminaccount aan in Supabase Auth en registreren tegelijk wie deze admin heeft toegevoegd.
                </p>
              </div>
          ...
          <aside className={cn('rounded-[30px] p-5 shadow-sm md:p-7', isDarkMode ? 'bg-[#18202b]' : 'bg-white')}>
            ...
            <div className={cn('mt-5 space-y-3 rounded-[28px] px-5 py-5', isDarkMode ? 'bg-[#111823]' : 'bg-[#f7f3eb]')}>
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--color-cozy-text)]">Waarom dit tabblad?</p>
                <p className={cn('mt-1 text-sm', isDarkMode ? 'text-[#a8b3c1]' : 'text-gray-500')}>Omdat je zo niet meer apart hoeft bij te houden wie welke admin heeft geregistreerd.</p>
              </div>
            </div>
          </aside>
        </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className={cn('mt-2 text-sm', isDarkMode ? 'text-[#b7c8c2]' : 'text-emerald-900/75')}>
                        Dit tijdelijke wachtwoord wordt enkel in deze sessie getoond. Kopieer het nu meteen door naar de juiste persoon.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <div className={cn('rounded-[22px] px-4 py-4', isDarkMode ? 'bg-black/20' : 'bg-white/80')}>
                      ...
                      <p className="mt-2 break-all font-mono text-lg font-bold text-[var(--color-cozy-text)]">{result.temporaryPassword}</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-5 space-y-3">
                {loading ? (
                  ...
                  visibleAdmins.map((account) => {
                    ...
                    const createdByCurrentAdmin = Boolean(adminEmail && account.createdByAdminEmail === adminEmail.trim().toLowerCase());
                    const isCurrentAdmin = Boolean(adminEmail && account.email === adminEmail.trim().toLowerCase());
                ...
                <div className="space-y-5">
                  ...
                  {selectedResult ? (
                    <div className={cn('rounded-[26px] border px-5 py-5', isDarkMode ? 'border-emerald-500/30 bg-[#162722]' : 'border-emerald-200 bg-emerald-50')}>
                      <p className="text-[10px] uppercase tracking-[0.22em] text-emerald-700">Tijdelijke login</p>
                      <p className="mt-3 text-sm font-medium text-[var(--color-cozy-text)]">
                        Dit is de admin die je net hebt aangemaakt. Het tijdelijke wachtwoord is alleen nu nog zichtbaar.
                      </p>
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        ...
                        <div className={cn('rounded-[20px] px-4 py-4', isDarkMode ? 'bg-black/20' : 'bg-white/80')}>
                          <p className="text-[10px] uppercase tracking-[0.2em] text-emerald-700">Tijdelijk wachtwoord</p>
                          <p className="mt-2 break-all font-mono text-sm font-bold text-[var(--color-cozy-text)]">{selectedResult.temporaryPassword}</p>
                        </div>
                      </div>
                      <button
                        type="button"
